...
@bot.on(register(outgoing=True, pattern=r"notifoff$"))
...
@bot.on(register(outgoing=True, pattern=r"notifon$"))
...
@bot.on(register(outgoing=True, pattern=r"(?:setuju|ok)\s?(.)?"))
async def approvepm(apprvpm):
    ...
    if apprvpm.reply_to_msg_id:
        ...
        if not isinstance(user, User):
            return await apprvpm.edit(
                "**Mohon Reply Pesan User Yang ingin diterima.**"
            )
    ...
    try:
        ...
        return await apprvpm.edit("**Pesan Lu Sudah Diterima Ya Tod, Silahkan Ngegibah anj**")

    await apprvpm.edit(
        f"**Menerima Pesan Cucu Dajjal** [{name0}](tg://user?id={uid})", 5
    )


@bot.on(register(outgoing=True, pattern=r"(?:tolak|nopm)\s?(.)?"))
...
@bot.on(register(outgoing=True, pattern=r"block$"))
...
@bot.on(register(outgoing=True, pattern=r"unblock$"))
...
@bot.on(register(outgoing=True, pattern=r"(set|get|reset) pmpermit(?: |$)(\w*)"))
